...
import com.bitgrind.kmp.filestorage.impl.SinkWriter
import com.bitgrind.kmp.filestorage.impl.SourceReader
import com.bitgrind.kmp.filestorage.impl.absolute
...
fun Sink.writer() = SinkWriter(this)

fun Source.reader() = SourceReader(this)
...
interface FileStorage {
    ...
    suspend fun <T> read(decoder: Decoder<T>, path: String): T {
        return decoder.decode(getReader(path))
    }

    suspend fun <T> write(encoder: Encoder<T>, path: String, value: T) {
        val writer = getWriter(path, append = false)
